...
class AssinaturaEstatutoController {
  ...
  getAllEstatutosAssinados(req, res) {
  ...
  getEstatutoAssinadosById(req, res) {
    const id = req.params.id;
    db.query("SELECT * FROM assinatura_estatuto WHERE id = ?", [id], (err, result) => {
      if (err) return res.status(500).json(err);
      res.status(200).json(result);
    });
  };

  getEstatutoAssinadosByUser(req, res) {
    const id = req.params.id;
    db.query("SELECT * FROM assinatura_estatuto WHERE id_usuario = ?", [id], (err, result) => {
      if (err) return res.status(500).json(err);
      res.status(200).json(result);
    });
  };
